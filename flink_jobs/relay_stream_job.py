...
#from flink_jobs.dynamodb_client import DynamoDBClientSingleton
...
def process_event(event):
    """Process each event and relay the event to the end user if connection is not paused"""
    event_data = json.loads(event)
    #dynamo_client = DynamoDBClientSingleton()
    #table = dynamo_client.get_table('events')
    #able.put_item(Item=event_data)
    return event


def relay_to_customer():
